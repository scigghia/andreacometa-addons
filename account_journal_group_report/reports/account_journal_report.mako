...
<center><H1>${_("ACCOUNT JOURNAL REPORT by GROUP")}</H1></center>
    ...
    <table class="font_9 w100 bordi_arrotondati">
        <tr>
			<td style="width:8%; float:left;">${_("DATE")}</td>
			<td style="width:8%; float:left;">${_("DOCUMENT")}</td>
			<td style="width:9%; float:left;">${_("REF")}</td>
			<td class="w40"><p style="text-align:left;">${_("PARTNER")}</p></td>
    ...
    <table id="content" class="font_9 w100">
		...
		%if move:    
			<tr>
				<td style="width:8%; float:left;">${move.date or ''}</td>
				<td style="width:8%; float:left;">${move.name or ''}</td>
				<td style="width:9%; float:left;">${move.ref or ''}</td>
				<td class="w40"><p style="text-align:left;">${move.line_id and move.line_id[0].partner_id.name or ''}</p></td>
		...
		<tr style="height:100%">
			...
			<td class="font_10" colspan="3"><b>${_("TOTAL:")} ${formatLang(tot_amount or 0.00, digits=get_digits(dp='Account'))}</b></td>
		</tr>
    </table>
