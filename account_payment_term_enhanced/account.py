...
class account_payment_term(osv.osv):
    ...
    _columns = {
        ...
        'days': fields.integer(
            'Number of Days', required=True,
            help="Number of days between every maturity from the day of month.\
If Date=15/01, Number of Days=22, Day of Month=-1, then the due date is 28/02."
        ),
        'days2': fields.integer(
            'Day of the Month', required=True,
            help="Day of the month, set -1 for the last day of the current mon\
th. If it's positive, it gives the day of the next month. Set 0 for net days (\
otherwise it's based on the beginning of the month)."),
        'maturities': fields.integer(
            "Number of maturities",
            help="Number of maturities to divide the total amount"),
        'start_day': fields.integer(
            'Start Day', required=True,
...
days (otherwise it's based on the beginning of the month)."),
        'monthly': fields.boolean(
            "Calculate by month",
            help="if checked, calculates one maturity for every month"),
    }
        ...
        if pt.fairly:
            ...
            if pt.start_day < 0:
                start_date = (
                    ...
                    relativedelta(days=pt.start_day))
            else:
                start_date = (
                    datetime.strptime(date_ref, '%Y-%m-%d') +
                    relativedelta(days=pt.start_day, months=0))
                ...
                if amt:
                    if line.start_day < 0:
                        start_date = (
                            datetime.strptime(date_ref, '%Y-%m-%d') +
                            relativedelta(day=1, months=1) +
                            relativedelta(days=line.start_day))
                    elif line.start_day > 0:
                        start_date = (
                            datetime.strptime(date_ref, '%Y-%m-%d') +
